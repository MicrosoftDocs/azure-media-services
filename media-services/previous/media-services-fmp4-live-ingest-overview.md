...
For certain critical live events that demand even higher availability and quality of experience, we suggest that you use active-active redundant encoders to achieve seamless failover with no data loss.
...
Use the following steps for ingesting sparse track:
...
1. Given the sparse nature of the signaling event, we suggest the following:
...
Use the following implementation for redundant audio tracks:
