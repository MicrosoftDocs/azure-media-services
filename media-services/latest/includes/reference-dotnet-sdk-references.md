---
author: IngridAtMicrosoft
ms.service: media-services 
ms.topic: include
ms.date: 08/20/2021
ms.author: inhenkel
---


<!--Additional .NET SDK reference links for use at the end of .Net articles and Tutorials/samples-->

## Additional .NET SDK references

* [Get started with the .NET SDK](../configure-connect-dotnet-howto.md)
* [More SDK samples](../samples-overview.md)
* [Encode and stream a remote file](../stream-files-dotnet-quickstart.md)
* [Encode and stream an uploaded file](../stream-files-tutorial-with-api.md)
* [Stream Live](../stream-live-tutorial-with-api.md)
* [Analyze your audio and video](../analyze-videos-tutorial.md)
